...
public interface UserMapper {
	...
	Manager selectByLoginnameAndPassword(
		...
		// 根据id查询用户
	@Select("select * from "+MANAGERTABLE+" where managerId = #{managerId}")
	Manager selectById(Integer managerId);
	
	// 根据id删除用户
	@Delete(" delete from "+MANAGERTABLE+" where managerId = #{managerId} ")
